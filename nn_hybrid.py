"""
nn_hybrid.py — MOTOR per-frame HÍBRIDO: signed regression + cabeza "is-stop" (no es entrypoint).
==================================================================================================
Arregla el colapso de STOP del modelo signed (model_06/08): la regresión-a-cero no se
detecta por umbral, así que le agregamos una cabeza de CLASIFICACIÓN binaria explícita
"¿este motor está parado?" entrenada con BCE — que SÍ tiene incentivo para detectar el cero.

Por motor, 2 salidas:
  - signed (tanh, PWM con signo)         -> dirección+magnitud CUANDO se mueve
  - is_stop (logit, binaria)             -> ¿está parado? (la autoridad sobre STOP)
...
La BCE usa pos_weight = #no_stop/#stop por motor (motor B casi no para -> pesa más).

Métricas (val): F1 macro de dirección decodificada (comparable con 00-08), MAE-PWM, full-stop recall.
Selección de mejor época por F1 macro. Reusa el Dataset signed de nn_signed.
"""
...
def decode_hybrid(signed_norm, stop_prob, eps_norm):
    """STOP si stop_prob>0.5; si no, dirección = signo del PWM. -> {0:STOP,1:FWD,2:BWD}."""
    out = signed_to_dir(signed_norm, eps_norm)   # 1/2 según signo (0 si |v|<eps)
...
# ==========================================
# GRAFICADO + EVALUACIÓN
# ==========================================
def plot_training_progress(metrics_csv, out_png, cfg=None):
    if not os.path.exists(metrics_csv):
        print(f"[WARN] No se encontró {metrics_csv}."); return
...
def evaluate_and_plot(model, loader, device, out_png, report_path=None, cfg=None):
    ...
    lines = ["===== MOTOR A (IZQ) — dirección (is-stop + signo) ====="]
    ...
    lines.append("\n===== MOTOR B (DER) — dirección (is-stop + signo) =====")
    ...
    fig.suptitle("model_09_hybrid — dirección = is-stop + signo del PWM")
...
def run_experiment(cfg, tag, results_dir="results"):
    ...
    save_config_snapshot(cfg, os.path.join(out, "config_used.txt"),
                         extra=f"# {tag} — PER-FRAME HÍBRIDO signed + is-stop (nn_hybrid.py)")

    print(f"=== {tag} | HÍBRIDO === Cargando CSVs...")
    ...
    # pesos L1 por dirección (suavizados) + pos_weight de la BCE is-stop por motor.
    ...
    # mejor época por F1 macro (default de make_trainer).
    ...
    print("Selección de mejor época por: f1 (val_dir_f1_macro, max)")

    print(f"\n🚀 ENTRENANDO {tag} (HybridControlNet) 🚀")
