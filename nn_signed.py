...
============================================================================================
Modelo: IMAGEN -> 2 variables CONTINUAS (signed_speedA, signed_speedB) ∈ [-100, 100]:
    > 0 FORWARD   < 0 BACKWARD   = 0 STOP   (la dirección es el SIGNO de la salida)

Diferencia con nn_perframe: NO hay cabezas de clasificación de dirección. Una sola salida
continua por motor (tanh*100) unifica dirección + velocidad en el comando físico (PWM).

PARA QUE BACKWARD SOBREVIVA (sin class-weights, porque no hay clases): la L1 se PONDERA
POR MUESTRA según la dirección REAL de cada motor (peso inverso a la frecuencia, igual
idea que las class-weights del modelo por-frame, pero aplicado al término L1 de cada motor).

MÉTRICAS (val, calculadas sobre la salida continua):
  - val_signed_mae_A/B/avg : MAE del PWM con signo (la métrica nativa; titular).
...
def signed_to_dir(v_norm, eps_norm):
    """Decodifica un PWM-con-signo NORMALIZADO (/100) a dirección {0:STOP,1:FWD,2:BWD}."""
...
class SignedDriveDataset(Dataset):
    ...
    def __getitem__(self, k):
        ...
        return img, {
            ...
            "dirA": torch.tensor(DIR_MAP[dirA], dtype=torch.long),  # solo para peso + métrica
...
class SignedControlNet(pl.LightningModule):
    def __init__(self, neck_hidden=(256,), dropout=0.4, backbone_frozen=True,
        ...
        # Pesos por dirección (STOP/FWD/BWD) para ponderar la L1 por muestra.
    ...
    def forward(self, x):
        ...
        # tanh acota la salida a [-1,1] -> *no* multiplicamos acá; el target está /100 en [-1,1].
        return torch.tanh(self.head_spdA(z)).squeeze(1), torch.tanh(self.head_spdB(z)).squeeze(1)

    def _weighted_l1(self, out, tgt):
        pA, pB = out
        wA = self.dir_weights[tgt["dirA"]]   # peso por la dirección REAL de cada motor
    ...
    def on_validation_epoch_end(self):
        ...
        # decodificar dirección desde el signo para F1 comparable con los modelos 00-05
...
# ==========================================
# 3. GRAFICADO + EVALUACIÓN
# ==========================================
def plot_training_progress(metrics_csv, out_png, cfg=None):
    if not os.path.exists(metrics_csv):
        print(f"[WARN] No se encontró {metrics_csv}.")
        return
    ...
    panels = [
        ...
        ("F1 macro dirección decodificada (Val)", [("val_dir_f1_A", "Motor A"), ("val_dir_f1_B", "Motor B")]),
...
def evaluate_and_plot(model, loader, device, out_png, report_path=None, cfg=None):
    ...
    lines = []
    lines.append("===== MOTOR A (IZQ) — dirección decodificada de sign(PWM) =====")
    ...
    lines.append("\n===== MOTOR B (DER) — dirección decodificada de sign(PWM) =====")
    ...
    for ax, (p, t, title) in zip(axes, [(pdA, tdA, "Motor A (IZQ)"), (pdB, tdB, "Motor B (DER)")]):
        ...
        ax.set_title(f"Confusión (dir decodificada) — {title}"); ax.set_xlabel("Predicho"); ax.set_ylabel("Real")
    fig.suptitle("model_06_signed — dirección = signo del PWM predicho")
...
def run_experiment(cfg, tag, results_dir="results", select_by="mae"):
    ...
    select_by: criterio para elegir el MEJOR checkpoint:
      "mae" -> menor val_signed_mae_avg (métrica física nativa).
      "f1"  -> mayor val_dir_f1_macro (dirección decodificada; comparable con 00-05).
    """
    ...
    # Pesos por dirección para la L1 ponderada (que BACKWARD sobreviva), SUAVIZADOS
    # por class_weight_power (1=inverso a freq, 0.5=raíz, 0=uniforme) para no aplastar STOP.
    ...
    print(f"Pesos L1 por dirección (power={power}) {[DIR_INV[i] for i in range(3)]}: {dir_weights.tolist()}")
    ...
    # Selección del mejor checkpoint según select_by. make_trainer ya viene con F1 (max);
    ...
    print(f"Selección de mejor época por: {select_by} "
          f"({checkpoint_callback.monitor}, mode={checkpoint_callback.mode})")

    print(f"\n🚀 ENTRENANDO {tag} (SignedControlNet) 🚀")
