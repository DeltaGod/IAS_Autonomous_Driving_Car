"""
model_09_hybrid.py — Modelo 9: PER-FRAME HÍBRIDO (signed + cabeza "is-stop").
=============================================================================
Intenta rescatar el STOP que el enfoque signed (model_06/08) no podía detectar.
Por motor: salida signed (tanh, PWM con signo) PARA cuando se mueve + cabeza binaria
"is-stop" (BCE) que decide explícitamente si está parado.
  DECODE: STOP si is-stop>0.5 ; si no, dirección = signo(signed), speed = |signed|.

Igual que model_08 (bases signed, arquitectura per-frame, pesos suavizados) + la cabeza
is-stop con su BCE (lambda_stop). Selección por F1 macro. Motor en nn_hybrid.py.
...
CFG = Config(
    ...
    # arquitectura/optimización (= model_08)
    ...
    class_weight_power=0.5,   # pesos L1 por dirección suavizados (= model_08)
