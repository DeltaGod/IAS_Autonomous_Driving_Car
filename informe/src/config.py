...
==================================================
Todo lo configurable vive aca. El resto de los programas importan `CFG` y usan
estos valores (NO hardcodear nada de esto en otro lado).

  from config import CFG
  CFG.lr, CFG.batch_size, CFG.resample_factor, ...

Corre este archivo directo para VER el efecto de tus parametros sobre la
distribucion del dataset (antes vs despues de los tratamientos):

  .venv/bin/python config.py        ->  genera 'augmentation_preview.png'

Asi tocas un numero aca, re-corres, y ves en el grafico como cambia el balance.
"""
...
class Config:
    ...
    # Factor de remuestreo por clase GLOBAL (cuantas muestras sinteticas generar):
    ...
    # Color jitter (robustez a iluminacion)
    ...
    # ===================== PERDIDA / BALANCEO =====================
    use_class_weights: bool = True    # pesos inversos a la frecuencia en la CE de direccion
    # Suaviza esos pesos elevandolos a esta potencia: 1.0 = inverso a freq (extremo),
    # 0.5 = raiz cuadrada (modera la clase rara: backward ~6x -> ~2.5x), 0.0 = uniforme.
    class_weight_power: float = 1.0
    lambda_speed: float = 1.0         # peso de la regresion de velocidad en la loss total
    lambda_stop: float = 1.0          # (modelo hibrido) peso de la BCE de la cabeza "is-stop"
    stop_pos_weight_cap: float = 0.0  # (hibrido) tope al pos_weight de la cabeza is-stop (0=sin tope)

    # ===================== ARQUITECTURA DEL MODELO =====================
    backbone_frozen: bool = True      # congelar el feature extractor de MobileNetV3
    # Cuello compartido: una entrada por capa oculta. no de capas = len(lista); neuronas = valores.
    #   [256]        -> 1 capa de 256
    ...
    # La ventana es MANY-TO-ONE: se predice la accion del ULTIMO frame de la
    # secuencia (causal y deployable: en el auto seria un buffer rodante).
    # seq_len y seq_stride son ADEMAS el experimento de "frecuencia de muestreo"
    # que pide el enunciado: a ~22fps, stride=3 ~= ~1s de contexto temporal.
    seq_len: int = 8          # no de frames por secuencia (ventana temporal)
    ...
    rnn_hidden: int = 128     # tamano del estado oculto recurrente
    rnn_layers: int = 1       # no de capas recurrentes apiladas
...
# =================================================================
# Resumen de parametros activos (para anotar en los graficos)
# =================================================================
...
# =================================================================
# Simulacion de la distribucion resultante (valor esperado)
# =================================================================
def simulate_distribution(df, cfg=CFG):
    """Calcula la distribucion de clases ANTES y DESPUES de aplicar:
    (1) resample_factor por clase global, (2) flip estocastico (swap A<->B y L<->R).
...
def plot_preview(cfg=CFG, out_png="augmentation_preview.png"):
    ...
    if not os.path.exists(cfg.train_csv):
        print(f"[ERROR] No existe {cfg.train_csv}. Corre build_global_csv.py primero.")
        return
    df = pd.read_csv(cfg.train_csv)
    dist = simulate_distribution(df, cfg)

    fig, axes = plt.subplots(1, 3, figsize=(20, 7))
    fig.suptitle("Distribucion del dataset de ENTRENAMIENTO: ANTES vs DESPUES de los tratamientos",
                 fontsize=15)
    titles = {"global": "Behavior GLOBAL (5 clases)",
              "motorA": "Direccion Motor A (IZQ)",
              "motorB": "Direccion Motor B (DER)"}
    ...
    for ax, key in zip(axes, ["global", "motorA", "motorB"]):
        ...
        ax.bar(x + 0.2, after, 0.4, label="Despues (config)", color="#DD8452")
    ...
    # Anotacion: QUE valores de las variables produjeron estos resultados.
    fig.text(0.5, 0.005, "Parametros activos:  " + active_params_text(cfg).replace("\n", "   |   "),
             ha="center", fontsize=10, family="monospace",
             bbox=dict(boxstyle="round", facecolor="#f0f0f0", edgecolor="gray"))
    plt.tight_layout(rect=[0, 0.05, 1, 1])
    plt.savefig(out_png, dpi=150)
    print(f"[graf] Preview guardado en '{out_png}'")
