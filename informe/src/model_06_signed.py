...
==================================================================
Reframe del target: en vez de (direccion 3-clases + velocidad) por motor, el modelo
predice UNA variable continua por motor -- el PWM con signo  in  [-100,100]:
    > 0 FORWARD   < 0 BACKWARD   = 0 STOP

Misma arquitectura/optimizacion que el per-frame (model_01) para comparar parejo; lo
unico distinto es el target (2 salidas continuas) y la loss (L1 ponderada por direccion
real de cada motor, para que BACKWARD no se muera al no haber class-weights).

Lee las bases SIGNED (generadas por build_signed_csv.py). Hiperparametros CONGELADOS abajo.
...
CFG = Config(
    ...
    # balanceo: peso por muestra en la L1 segun direccion (que BACKWARD sobreviva)
