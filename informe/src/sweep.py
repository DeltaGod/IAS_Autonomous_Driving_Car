"""
sweep.py -- Exploracion de hiperparametros GENERICA (frontera de Pareto + anti-ruido).
=====================================================================================
Barre una grilla de hiperparametros sobre CUALQUIER modelo (hybrid / perframe / signed /
...
QUE PRODUCE (en results/sweeps/<nombre>/):
  - <NN>_<tag>/            : carpeta de resultados de cada config (eval, report, metrics, ckpt)
  - sweep_summary.csv      : una fila por config con sus hiperparametros + metricas del MEJOR
                             checkpoint (el que elige el modelo: max val_dir_f1_macro).
  ...
  - param_effects.png      : efecto marginal de cada parametro sobre F1 y full-stop.
  - reseed_summary.csv     : ANTI-RUIDO. Re-corre los finalistas (los de Pareto) con varias
                             semillas y reporta media +/- desvio -> la ventaja sobrevive al
...
# nombre de motor -> (modulo, funcion run_experiment)
ENGINES = {
...
def best_operating_point(metrics_csv, select="val_dir_f1_macro", mode="max"):
    """Metricas en la epoca del MEJOR checkpoint (el criterio con que el modelo elige)."""
    if not metrics_csv or not os.path.exists(metrics_csv):
        return None
    m = pd.read_csv(metrics_csv)
    g = m.groupby("epoch").max(numeric_only=True)   # junta filas train/val de cada epoca
...
def pareto_mask(xs, ys):
    """True si el punto NO esta dominado (ambos ejes a MAXIMIZAR)."""
...
def launch(engine, tag, results_dir, seed, cfg):
    """Corre una config en subproceso; devuelve (returncode, metricas-del-mejor-ckpt)."""
...
def run_sweep(name, engine, base, grid, axis_x="val_dir_f1_macro",
    ...
    if ok.empty:
        print("[sweep] ninguna config produjo metricas; abortando plots."); return
    ok = ok.copy()
...
def plot_pareto(df, ax_x, ax_y, out_png, name):
    ...
    print(f"[graf] Pareto guardado en '{out_png}'")
...
def plot_param_effects(df, keys, ax_x, ax_y, out_png):
    ...
    fig.suptitle("Efecto marginal de cada parametro (promediando el resto)", fontsize=13)
    plt.tight_layout(rect=[0, 0, 1, 0.96]); plt.savefig(out_png, dpi=150); plt.close(fig)
    print(f"[graf] Efectos por parametro en '{out_png}'")


# =====================================================================
# DEFINICION DEL SWEEP (edita aca)
# =====================================================================
def model09_sweep():
    base = Config(
    ...
    grid = {                                   # Tier 1 (knobs de la falla) + Tier 2 (capacidad)
        ...
        "dropout": [0.4, 0.5],                 # regularizacion
    }                                          # 3*3*2*2 = 36 configs
...
def model01_sweep():
    """Sweep del per-frame (model_01, el mejor): arquitectura + regularizacion + optimizacion."""
    ...
    grid = {
        "neck_hidden": [[256], [512, 256]],        # capacidad del cuello
        "dropout": [0.3, 0.4, 0.5],                # regularizacion
        "lr": [5e-4, 1e-3],                        # optimizacion
        "weight_decay": [1e-4, 1e-3],              # regularizacion L2
    }                                              # 2*3*2*2 = 24 configs
...
def smoke_sweep():
    """Mini-sweep para validar el pipeline end-to-end (2 configs, 2 epocas, 1 reseed)."""
    spec = model09_sweep()
...
def main():
    ...
    ap.add_argument("--which", default="m09", choices=list(SWEEPS))  # que sweep correr
