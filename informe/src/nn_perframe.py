...
Modelo: IMAGEN (1 frame) -> control por motor (4 salidas):
  - behaviorA, behaviorB  in  {STOP, FORWARD, BACKWARD}  (clasificacion, 3 clases)
  - speedA, speedB  in  [0,100]                           (regresion, normalizada a [0,1])
...
que arma una `Config` CONGELADA y llama aca. Asi "se ve el modelo en cada codigo"
sin duplicar el train-loop. Ver convencion en `results/README.md`.
...
class MotorControlNet(pl.LightningModule):
    ...
    def on_validation_epoch_end(self):
        ...
        # recall de full-stop conjunto (ambos motores STOP a la vez) -- metrica de seguridad
...
def predictions_to_control(dirA, dirB, speedA, speedB, speed_eps=1.0):
    """Traduce la salida abstracta a pines del L298N (estados siempre validos)."""
    if speedA < speed_eps:
        dirA = "STOP"
    if speedB < speed_eps:
        dirB = "STOP"
...
# ==========================================
# 4. GRAFICADO + EVALUACION LOCAL
# ==========================================
def plot_training_progress(metrics_csv, out_png="training_progress.png", cfg=None):
    if not os.path.exists(metrics_csv):
        print(f"[WARN] No se encontro {metrics_csv}; no se grafica el progreso.")
        return
...
def save_config_snapshot(cfg, path, extra=""):
    """Vuelca los hiperparametros CONGELADOS de este experimento a un .txt legible."""
...
def run_experiment(cfg, tag, results_dir="results"):
    ...
    print(f"\n[run] ENTRENANDO {tag} (MotorControlNet) [run]")
