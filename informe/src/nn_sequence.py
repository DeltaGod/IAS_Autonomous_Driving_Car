...
=========================================================================================
Modelo: SECUENCIA de imagenes -> control por motor (mismas 4 salidas que nn_perframe).

POR QUE una recurrente: el error dominante del per-frame es STOP vs FORWARD, porque la
VELOCIDAD no esta en un unico frame (un auto quieto y uno avanzando despacio en el mismo
punto son identicos). Una secuencia muestra si la escena avanza -> la GRU/LSTM resuelve
esa ambiguedad temporal.
...
logica de loss/metricas/eval/plots de `nn_perframe` (herencia) para que la comparacion
...
# Reutilizamos del modelo por-frame: mapeos, loss/metricas (herencia), eval, plots, utils.
...
class SequenceDriveDataset(Dataset):
    def __init__(self, df, is_train=True, cfg=None):
        ...
        # Ventanas validas: una por cada frame que tenga (seq_len-1)*stride frames previos
    ...
    def _build_samples(self):
        ...
        # el solape entre secuencias casi identicas (sliding window a 22fps). Val sin tocar.
        ...
        for _, g in self.df.groupby("record", sort=False):
            ...
            for pos in range(span, len(idxs), wstep):
                win = idxs[pos - span: pos + 1: k]  # T_ indices, orden temporal ascendente
                if len(win) == T_:
                    base.append(win)
        if not self.is_train:
            return base
        # Resample por el behavior global del frame-ETIQUETA (el ultimo de la ventana).
    ...
    def label_frames(self):
        """DataFrame de los frames-etiqueta (ultimo de cada ventana, post-resample).
...
# ==========================================
# 2. MODELO CNN + GRU/LSTM (hereda loss/metricas de MotorControlNet)
# ==========================================
class MotorControlGRU(MotorControlNet):
    ...
    / configure_optimizers de MotorControlNet (misma loss y misma metrica F1)."""
    ...
    def __init__(self, neck_hidden=(256,), dropout=0.4, rnn_type="GRU", rnn_hidden=128,
        ...
        # Referenciar __class__ fuerza la creacion de la celda __class__ del metodo: sin
        # ella, save_hyperparameters de Lightning 2.x no reconoce este __init__ como el de
        # una clase y NO captura ningun hparam (quedaria hparams.lr vacio -> KeyError).
        ...
        # Cuello compartido sobre el ultimo estado oculto (igual idea que el por-frame).
        layers, d = [], rnn_hidden
    ...
    def forward(self, x):  # x: (B, T, C, H, W)
        ...
        # lo pondria en modo train cada epoca). Solo entrenan rnn + neck + cabezas.
        self.backbone.eval()
        with torch.no_grad():
        ...
        z = self.neck(out[:, -1])          # ultimo timestep -> many-to-one
...
def run_experiment(cfg, tag, results_dir="results"):
    ...
    if len(val_ds) < 50:
        print(f"[ADVERTENCIA] Solo {len(val_ds)} secuencias de validacion: "
              f"la metrica va a ser RUIDOSA (val es corto y T/stride lo reducen mas).")
    ...
    print(f"\n[run] ENTRENANDO {tag} (MotorControlGRU / {cfg.rnn_type}) [run]")
