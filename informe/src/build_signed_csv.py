...
En vez de (direccion {STOP,FWD,BWD} + velocidad [0,100]) por motor, el target es UNA
variable continua por motor: el PWM con SIGNO  in  [-100, 100]:
    > 0  -> FORWARD     < 0  -> BACKWARD     = 0  -> STOP

POR QUE ENVUELVE build_global_csv (y no reimplementa):
toda la logica delicada (regla de freno, asociacion comando->imagen anterior, recorte
de bordes, filtro de latencia >1s con segmentacion en _seqN, interpolacion + pick_dir,
decode por motor) YA esta validada ahi. Aca la reusamos tal cual y solo TRANSFORMAMOS
el target al final:
...
direccion. No se interpola "a traves del cero" (eso seria reimplementar la interpolacion);
la diferencia solo afecta reversas FWD->BWD sin STOP intermedio, casi inexistentes aca.
...
def dump_split(rows, path, name):
    ...
    if not len(out):
        return
    # Verificacion: distribucion del signo por motor (debe espejar la direccion).
...
def main():
    ...
    ap.add_argument("--output-val", default="dataset_val_signed.csv", help="CSV de validacion SIGNED")
    args = ap.parse_args()
    ...
    # MISMO split por sesion que build_global_csv: 1a sesion (la grande) -> TRAIN, resto -> VAL.
