...
IMAGEN -> direccion {STOP,FORWARD,BACKWARD} + velocidad, por motor (A y B).

Random Forest sobre imagenes en grises 32x32 aplanadas:
  - 2 clasificadores de direccion (motor A, motor B), class_weight='balanced'.
  - 2 regresores de velocidad (motor A, motor B), MAE como metrica.
Split por sesion ya materializado: dataset_train.csv / dataset_val.csv.

Hiperparametros CONGELADOS abajo (esto ES el modelo). NO usa Lightning ni GPU.
...
# ===== hiperparametros CONGELADOS de este modelo =====
TRAIN_CSV = "dataset_train.csv"
VAL_CSV = "dataset_val.csv"
IMG_SIZE = (32, 32)        # imagenes en grises, aplanadas a 1024 features
N_ESTIMATORS = 100
MAX_DEPTH = 15
RANDOM_STATE = 42
...
def load_dataset(csv_path, img_size=IMG_SIZE):
    """Carga imagenes (grises, aplanadas) y los targets por motor."""
    df = pd.read_csv(csv_path)
    X, dirA, dirB, spdA, spdB = [], [], [], [], []
    print(f"Cargando {len(df)} imagenes de {csv_path} a {img_size}...")
    start = time.time()
...
def run_experiment(results_dir="results"):
    ...
    for name, y_tr, y_va in [("A (IZQ)", dirA_tr, dirA_va), ("B (DER)", dirB_tr, dirB_va)]:
        ...
        report_lines.append(f"===== DIRECCION MOTOR {name} =====\n{rep}")
    ...
    # guardar matrices de confusion por motor (mismo formato que los modelos deep)
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    for ax, name in zip(axes, ["A (IZQ)", "B (DER)"]):
        sns.heatmap(cms[name], annot=True, fmt='d', cmap='Blues',
                    xticklabels=LABELS, yticklabels=LABELS, ax=ax)
        ax.set_title(f"Confusion -- Motor {name}"); ax.set_xlabel("Predicho"); ax.set_ylabel("Real")
